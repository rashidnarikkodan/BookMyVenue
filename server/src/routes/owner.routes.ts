...
import { validateRequest, validateObjectId, validateQuery } from "@/middlewares/validate.middleware";
...
router
  .route("/venues")
  .post(validateRequest(createVenueSchema), venueController.createVenue)
...
router
  ...
  .patch(validateRequest(updateVenueSchema), venueController.updateVenue);
